...
class AutosarAPDocumentDownloader:
    def __init__(self):
        # 从output文件夹读取JSON文件
        self.output_dir = "output"
        self.documents_file = os.path.join(self.output_dir, "autosar_ap_documents.json")
        # 使用download_ap作为下载目录
        self.download_dir = "download_ap"
        self.headers = {
        ...
        self.max_retries = 3
        self.download_log = os.path.join(self.download_dir, "download_ap_log.txt")
    ...
    def download_all(self):
        ...
        print(f"开始下载 {len(documents)} 个AUTOSAR AP文档...")
        
        # 初始化下载日志
        with open(self.download_log, 'w', encoding='utf-8') as f:
            f.write(f"AUTOSAR AP文档下载日志 (共 {len(documents)} 个)\n")
            f.write("=" * 80 + "\n\n")
...
if __name__ == "__main__":
    downloader = AutosarAPDocumentDownloader()
