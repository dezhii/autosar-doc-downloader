...
class AutosarAPDocumentCollector:
    def __init__(self):
        ...
        self.output_file = os.path.join(self.output_dir, "autosar_ap_documents.txt")
        self.json_output_file = os.path.join(self.output_dir, "autosar_ap_documents.json")
        self.target_count = 61  # AP平台文档数量
        self.max_retries = 3
    ...
    def get_page(self, page_number=1, retries=0):
        ...
        try:
            ...
            if page_number == 1 or len(self.parse_documents(response.text)) == 0:
                debug_file = os.path.join(self.output_dir, f"autosar_ap_page_{page_number}.html")
    ...
    def collect_documents(self):
        """收集所有文档"""
        print(f"开始收集AUTOSAR AP文档，目标数量: {self.target_count}")
    ...
    def try_alternative_search(self):
        ...
        try:
            ...
            alternative_params = [
                ...
                {"tx_solr[filter][0]": "platform:AP", "tx_solr[q]": ""},
                {"tx_solr[q]": "AUTOSAR AP R24-11"}
            ]
    ...
    def save_documents(self):
        ...
        with open(self.output_file, 'w', encoding='utf-8') as f:
            f.write(f"AUTOSAR AP文档列表 (共 {len(self.documents)} 个)\n")
            f.write("=" * 80 + "\n\n")
...
if __name__ == "__main__":
    collector = AutosarAPDocumentCollector()
